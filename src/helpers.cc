...
KD_Node* generate_KD_Tree(GLViewKD_Trees* glview, Vector pos, std::vector<Vector> verts, Vector min, Vector max, std::map<WO*, KD_Node*>& PlaneMap, int iteration) {
	...
	if (iteration % 3 == 0) { // x plan
        ...
        PlaneMap.insert(std::pair<WO*, KD_Node*>(new_plane, new_node));

        iteration--;
        new_node->left = generate_KD_Tree(glview, pos, v1, v1_min, v1_max, PlaneMap, iteration);
        new_node->right = generate_KD_Tree(glview, pos, v2, v2_min, v2_max, PlaneMap, iteration);
	}
